...
`;let w={loading:!1,projectDetail:{},otherProducts:[],count:1};const T=async()=>{let[,e,t]=location.pathname.replace(`/front_6th_chapter1-1`,``).split(`/`);if(console.log(`111111111111111111`),console.log(location.pathname.replace(`/front_6th_chapter1-1`,``).split(`/`)),console.log(e),t){w.loading=!0,document.body.querySelector(`#root`).innerHTML=C(w);let e=await a(t),n=e.category1,r=await i({category1:n}),o=r.products.filter(({productId:e})=>e!==t);w.loading=!1,w.projectDetail=e,w.otherProducts=o,document.body.querySelector(`#root`).innerHTML=C(w)}else return x()};function E(){w.count+=1,document.body.querySelector(`#root`).innerHTML=C(w)}function D(){--w.count,document.body.querySelector(`#root`).innerHTML=C(w)}document.addEventListener(`click`,e=>{e.target.closest(`#quantity-increase`)&&E(e),e.target.closest(`#quantity-decrease`)&&D(e)});const O=document.querySelector(`#root`);O.addEventListener(`click`,e=>{let t=e.target.closest(`.related-product-card`);if(!t)return;e.preventDefault();let n=t.dataset.productId;history.pushState({},``,`/product/${n}`),window.dispatchEvent(new PopStateEvent(`popstate`))});const k=()=>r(async()=>{let{worker:e}=await import(`./browser-CcyfQrG1.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`,serviceWorker:{url:`./mockServiceWorker.js`}}));window.addEventListener(`popstate`,A),window.addEventListener(`hashchange`,A);function A(){let[,e,t]=location.pathname.replace(`/front_6th_chapter1-1`,``).split(`/`);if(e===`product`&&t)return console.log(`project detail page`),T(t);if(console.log(e),!e)return p();document.body.querySelector(`#root`).innerHTML=x()}k().then(A);