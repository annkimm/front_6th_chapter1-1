...
export const searchBar = (
  ...
  return /* HTML*/ `        
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
          ...
          <div class="mb-4">
            <div class="relative">
              <input type="text" id="search-input" placeholder="상품명을 검색해보세요..." value="${filter.search || ""}" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg
          ...
          <div class="space-y-3">
            ...
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                <label class="text-sm text-gray-600">카테고리:</label> <button data-breadcrumb="reset" class="text-xs hover:text-blue-800 hover:underline">전체</button>${(filter.category1 ?? "") !== "" ? `<span class="text-xs text-gray-500">&gt;</span><button data-breadcrumb="category1" data-category1="${filter.category1}" class="text-xs hover:text-blue-800 hover:underline">${filter.category1}</button>` : ""}${(filter.category2 ?? "") !== "" ? `<span class="text-xs text-gray-500">&gt;</span><span class="text-xs text-gray-600 cursor-default">${filter.category2}</span>` : ""}</div> <div class="flex flex-wrap gap-2">${
                  loading
